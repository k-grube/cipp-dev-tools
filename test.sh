#!/usr/bin/env bash
# forwards to frontend-tests npm scripts: ./test.sh [--watch|--unit|--storybook|--browser] [vitest args]
set -euo pipefail
...
for a in "$@"; do
    case "$a" in
        ...
        --help|-h)
            echo 'usage: ./test.sh [--watch|--unit|--storybook|--browser] [vitest args, e.g. a test file or -t "name"]'
