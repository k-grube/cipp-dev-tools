...
- `to_json(G, communities, path, force=, community_labels=)` returns False (writing nothing) when the new graph has fewer nodes than the existing file unless force=True (#479). update.py/routelink/craftlink wrap this with their own 10 percent threshold + force=True (build_merge fuzzy dedup can shrink a merge by a few nodes, force=False would silently drop the pass); recluster uses force=False deliberately (node count never changes)
