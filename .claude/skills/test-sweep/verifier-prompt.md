...
4. rejected tests: delete them from the file yourself (whole `it()` blocks; delete the file if nothing survives). rerun the file if you edited it: `npx vitest run --project {{project}} <testPath>` from `CIPP\frontend`.
